...
class main(FloatLayout):
    def __init__(self, **kwargs):
        ...
        self.fish = Stockfish(
            "./stockfish", parameters={"Minimum Thinking Time": 1000, "Slow Mover": 10})
        ...
        self.info = Label(text=self.info_text, size_hint_y=1, size_hint_x=1,
                          markup=True, text_size=(545, 100), pos=(362, 45), valign="top")
        self.add_widget(self.info)

        self.move_options = Label(text=self.moves_string, markup=True, text_size=(
            545, 500), pos=(362, -317), valign="top")
    ...
    def evaluate_resign(self):
        try:
            if self.board_evaluations[-1]["type"] == "mate":
                result = (self.is_white and self.board_evaluations[-1]["value"] > 0) or (
                    not self.is_white and self.board_evaluations[-1]["value"] < 0)
                if result:
    ...
    def update_history(self, reset=False):
        if reset:
            ...
            if self.is_white:
                broadcast(
                    poll_message, "A new game has started against %s, chat is white" % opp)
                self.game_history.headers["White"] = "Twitch chat"
                self.game_history.headers["Black"] = opp
            else:
                broadcast(
                    poll_message, "A new game has started against %s, chat is black" % opp)
            ...
            if self.last_game_node is None:
                self.last_game_node = self.game_history.add_main_variation(
                    self.board.move_stack[-1])
            else:
                self.last_game_node = self.last_game_node.add_main_variation(
                    self.board.move_stack[-1])
    ...
    def update_info(self, dt=0, text=None, hold=False):
        ...
        if text is None:
            ...
            if c is not None and "challenger" in c:
                ...
                skill = db.get_level()
                self.info_text = "Opponent: Stockfish lvl %d is %s, ELO %d~" % (
                    skill, opp_color, int(1000 + skill * 90))
                ...
                if self.board_evaluations[-1]["type"] == "mate":
                    if self.board_evaluations[-1]["value"] > 0:
                        ...
                        self.info_text += "\nBlack mate in %d" % (
                            self.board_evaluations[-1]["value"] * -1)
                else:
                    self.info_text += "\nEval: %.2f" % (
                        self.board_evaluations[-1]["value"] / 100)

            self.info_text += ", Game %d, W:%d, D:%d, L:%d" % (
                self.round, self.record[0], self.record[1], self.record[2])
            if self.countdown > 0:
                self.countdown -= dt
                if self.countdown < 0:
                    self.countdown = 0
    ...
    def update_board(self):
        image = self.renderer.draw(
            self.board.fen(), self.is_white, lastmove=self.lastmove)
        data = BytesIO()
    ...
    def fish_move_(self, dt):
        ...
        broadcast(poll_message, "Stockfish went %s" %
                  self.board.san(chess.Move.from_uci(self.lastmove)))
    ...
    def end_game(self, result):
        ...
        if result == "w":
            ...
            self.update_info(
                text="You drew, 50 points awarded to participants", hold=True)
        elif result == "a":
    ...
    def set_legal_moves(self, end=False):
        ...
        if len(legal) == 0 or (type(end) == bool and end):
            ...
            self.moves_string = self.format_text(
                "VIP leaderboard, use !vip to climb", font_size=30)
            temp = ""
            for i in range(10):
                temp += "\n%d. %s - %d points" % (
                    vips[i][2], vips[i][0], vips[i][1])
        ...
        for move in legal:
            temp = set()
            san = self.board.san(move.from_uci(move.uci())).replace(
                "+", "").replace("#", "")
            if san in moves:
            ...
            temp.add(move.uci())
            temp.add(san)
            temp.add(rchop(san, "=Q").replace(
                "O", "0").casefold().replace("x", ""))
        ...
        self.moves_string = self.format_text("Legal moves, type in chat to vote, eg.%s or 1:\n" % list(
            notation_moves_temp)[0], font_size=font_size[0]) + self.format_text(movelist, font_size=font_size[1])
        self.move_options.text = self.moves_string
        voted.set(set())
        vetoed.set(set())
...
async def command_pgnplay(ctx):
    ws = bot._ws
    await ws.send_privmsg("#%s" % ctx.channel, f"/me PGN viewer and FEN editor here: https://chess.com/analysis")
