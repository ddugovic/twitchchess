...
class DrawChessPosition(object):
    ...
    def draw(self, fen, white, lastmove=None):
        ...
        if not lastmove is None:
            ...
            ImageDraw.Draw(board).rectangle((self.square(letters.index(lastmove[2]), numbers.index(
                lastmove[3])), self.square(letters.index(lastmove[2]) + 1, numbers.index(lastmove[3]) + 1)), fill="gray")
        ...
        for i in range(n):
            ImageDraw.Draw(board).text(
                self.bot(i, 7), letters[i], font=fnt, fill="black")
            ImageDraw.Draw(board).text(self.point(0, i),
                                       numbers[i], font=fnt, fill="black")
        return board
